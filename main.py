...
def run_pipeline(workspace: str = "default", sample_path: str = "sample_data/raw_inputs.json") -> dict:
    workspace_ctx = WorkspaceManager().resolve(workspace)
    raw_items = IngestorAgent().load_json(sample_path)
    result = MultiAgentRuntime(workspace_ctx).run(raw_items)
    return result


if __name__ == "__main__":
    args = build_args()
    output = run_pipeline(workspace=args.workspace, sample_path=args.input)
